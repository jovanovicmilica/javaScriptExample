...
if(url!='/javaScriptExample/index.html' && url!='/javaScriptExample/'){
    let curentUrl=url.split('/javaScriptExample/')[1];
...
function getMovies(movies,images,divForPrint){
    ...
    for(let i=0;i<movies.length;i++){
        print+=`<div class="col-5 col-md-3 movie`;
                ...
                if(url=='/javaScriptExample/movies.html'){
                    print+=` mx-3`;
                }
...
if(url=='/javaScriptExample/theatres.html'){
    var theatres=['Regal Manassas','Regal Virginia Gateway','Regal Fox 4DX & IMAX','Regal Fairfax Towne Center','Regal Dulles Town Center'];
...
if(url=='/javaScriptExample/events.html'){
...
if(url=='/javaScriptExample/contact.html'){
    getTheatresContact();
    getSlider();
}
...
function checkName(){
    let nameRegex=/^[A-ZZČĆŠĐŽ][a-zčćšđž]{2,}\s[A-ZZČĆŠĐŽ][a-zčćšđž]{2,}(\s[A-ZZČĆŠĐŽ][a-zčćšđž]{2,})?$/;
